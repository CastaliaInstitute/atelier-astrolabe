...
static const char *TAG = "faculty18";

#define WIFI_CONNECTED_BIT BIT0
#define WIFI_FAIL_BIT BIT1

static EventGroupHandle_t s_wifi_events;
static faculty18_ui_state_t s_ui = FACULTY18_UI_BOOT;
...
{
    (void)ctx;
    if (frame == NULL || frame_samples == 0 || s_voice_queue == NULL) {
        return;
    }
...
typedef struct {
    faculty18_ui_state_t state;
    char detail[96];
} atom_ui_msg_t;

static void ui_set(faculty18_ui_state_t state, const char *detail)
{
    s_ui = state;
    if (detail != NULL) {
...
static void ui_task(void *arg)
{
    ...
    while (true) {
        ...
        faculty18_display_draw_status(s_ui, s_faculty_name, s_detail);
    }
}
...
static void handle_voice_result(uint32_t turn, uint32_t t0, atom_voice_result_t *result)
{
    ...
    if (result->mp3 != NULL && result->mp3_len > 0) {
        ui_set(FACULTY18_UI_SPEAK, result->faculty_name[0] ? result->faculty_name : s_faculty_name);
    ...
    ui_set(FACULTY18_UI_LISTEN, NULL);
    ...
    while (true) {
        ...
        switch (ev.type) {
            ...
            case VOICE_EVT_END: {
                ...
                const float dur_s = (float)ev.sample_count / (float)FACULTY18_AUDIO_RATE;
                ATOM_LOG_STAGE(TAG, "turn", "#%u streamed %.2fs (%u samples)", (unsigned)turn, dur_s,
                               (unsigned)ev.sample_count);
                ui_set(FACULTY18_UI_THINK, "Castalia…");
                ATOM_LOG_STAGE(TAG, "pipeline", "STT->ask-faculty->Chirp3 TTS (face=%s)", ASTROLABE_FACULTY_FACE_NAME);
                ...
                if (err != ESP_OK) {
                    const int http_st = atom_voice_last_http_status();
                    ATOM_LOG_STAGE_E(TAG, "pipeline", "turn #%u failed after %ums http=%d err=%s", (unsigned)turn,
                                     (unsigned)(atom_log_ms() - t0), http_st, esp_err_to_name(err));
                    if (err == ESP_ERR_INVALID_SIZE || http_st == 422) {
                        ATOM_LOG_STAGE_W(TAG, "pipeline", "no speech detected — listening");
                        ui_set(FACULTY18_UI_LISTEN, NULL);
                    } else {
                        char detail[32];
                        if (http_st > 0) {
                            snprintf(detail, sizeof(detail), "HTTP %d", http_st);
                        } else {
                            atom_strlcpy(detail, "voice fail", sizeof(detail));
                        }
                        ui_set(FACULTY18_UI_ERROR, detail);
...
static void listen_task(void *arg)
{
    ...
    while (true) {
        if (s_ui == FACULTY18_UI_SPEAK || s_ui == FACULTY18_UI_THINK) {
            was_capturing = false;
            capture_clear_ms = 0;
        ...
        size_t got = 0;
        if (!faculty18_board_audio_ready()) {
            vTaskDelay(pdMS_TO_TICKS(100));
            continue;
        }
        if (faculty18_audio_read(frame, ATOM_LISTEN_FRAME_SAMPLES, &got, 100) != ESP_OK || got == 0) {
            vTaskDelay(1);
            continue;
        }
        ...
        if ((s_ui == FACULTY18_UI_LISTEN || s_ui == FACULTY18_UI_CAPTURE) && capturing && !was_capturing) {
            ATOM_LOG_STAGE(TAG, "capture", "speech detected — hearing");
            ui_set(FACULTY18_UI_CAPTURE, NULL);
            was_capturing = true;
            capture_clear_ms = 0;
        } else if ((s_ui == FACULTY18_UI_LISTEN || s_ui == FACULTY18_UI_CAPTURE) && !capturing && was_capturing) {
...
static void faculty_log_ready(void)
{
    ...
    ATOM_LOG_STAGE(TAG, "ready", "pipeline %s/functions/v1/voice-pipeline -> ask-faculty (Chirp3 TTS)",
                   MYNAH_SUPABASE_URL);
    ATOM_LOG_STAGE(TAG, "ready", "listening — speak to run STT->LLM->TTS");
    ATOM_LOG_STAGE(TAG, "ready", "serial: help | qa audio | ./scripts/faculty18_screenshot.py -p PORT");
    ATOM_LOG_STAGE(TAG, "ready", "monitor: ./scripts/faculty18_build.sh -p PORT monitor");
}
...
static esp_err_t wifi_start(void)
{
    ...
    ui_set(FACULTY18_UI_WIFI, MYNAH_WIFI_SSID);
...
void app_main(void)
{
    ATOM_LOG_STAGE(TAG, "boot", "Astrolabe Faculty — Waveshare ESP32-S3 Touch AMOLED 1.8");

    ESP_ERROR_CHECK(nvs_flash_init());
    load_faculty_from_nvs();
    ATOM_LOG_STAGE(TAG, "boot", "faculty %s (%s)", s_faculty_name, s_faculty_slug);

    ESP_ERROR_CHECK(faculty18_board_init());
    atom_serial_init();
    ATOM_LOG_STAGE(TAG, "boot", "board audio=%s", faculty18_board_audio_ready() ? "ok" : "off");
    ...
    xTaskCreate(ui_task, "ui", 10240, NULL, 6, NULL);
    ui_set(FACULTY18_UI_BOOT, ASTROLABE_FACULTY_OTA_CHANNEL);

    if (wifi_start() != ESP_OK) {
        ui_set(FACULTY18_UI_ERROR, "wifi");
        ATOM_LOG_STAGE_W(TAG, "wifi", "continuing offline — bust may load, voice will fail");
    }
    ...
    xTaskCreate(listen_task, "listen", 6144, NULL, 4, NULL);
    ...
    while (true) {
        ...
        if (faculty18_button_just_pressed()) {
            atom_strlcpy(s_faculty_slug, ASTROLABE_FACULTY_DEFAULT_FACULTY_SLUG, sizeof(s_faculty_slug));
            atom_strlcpy(s_faculty_name, ASTROLABE_FACULTY_DEFAULT_FACULTY_NAME, sizeof(s_faculty_name));
            s_history[0] = '\0';
            save_faculty_to_nvs();
