...
static const char *ui_state_name(faculty18_ui_state_t state)
{
    switch (state) {
...
static void qa_emit_status(void)
{
    ...
    printf("qa: face=faculty lcd=%dx%d audio=%s pi4ioe=%s mic_probe_peak=%ld heap=%u psram=%u wifi_rssi=%d ch=%u\n",
           FACULTY18_LCD_W,
           FACULTY18_LCD_H,
           faculty18_board_audio_ready() ? "ok" : "off",
           faculty18_board_pi4ioe_ok() ? "ok" : "fail",
           (long)faculty18_board_mic_probe_peak(),
...
{
    const faculty18_ui_state_t ui = s_bind.ui != NULL ? *s_bind.ui : FACULTY18_UI_BOOT;
