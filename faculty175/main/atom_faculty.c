...
static bool build_supabase_bust_url(const char *slug, char *url, size_t cap)
{
    ...
    base[sizeof(base) - 1] = '\0';
    trim_base_url(base);
    const int n = snprintf(url, cap, "%s/functions/v1/faculty-bust?faculty=%s&w=%d&h=%d&q=60&resize=cover&view=right",
...
/** Drop fringe/letterbox pixels after PNG alpha (matches watch pm_faculty.cpp). */
...
static bool bust_cache_path(const char *slug, char *path, size_t cap)
{
    ...
    const int n = snprintf(path, cap, BUST_CACHE_BASE "/%s-right.png", slug);
    return n > 0 && (size_t)n < cap;
}
...
static void prefetch_roster_task(void *arg)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (!atom_faculty_roster_get(i, &entry)) {
            continue;
        }
                                   ...
                                   int *out_y)
{
    ...
    int bcx = area_w / 2;
    int bcy = area_h / 2;
    if (!atom_faculty_bust_content_center(&bcx, &bcy)) {
        *out_x = area_x;
        *out_y = area_y;
        return;
    }
