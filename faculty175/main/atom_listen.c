...
static void waveform_push(atom_listen_t *listen, uint8_t level, bool streamed)
{
    if (listen == NULL) {
        return;
    }
...
void atom_listen_waveform_stream_copy(const atom_listen_t *listen, uint8_t *out, size_t len)
{
    if (listen == NULL || out == NULL || len == 0) {
        return;
    }
    ...
    const uint16_t head = listen->waveform_head;
    for (size_t i = 0; i < n; ++i) {
        out[i] = listen->waveform_stream[(head + i) % ATOM_LISTEN_WAVEFORM_LEN];
    }
    if (len > n) {
        memset(out + n, 0, len - n);
    }
...
bool atom_listen_push_frame(atom_listen_t *listen, const int16_t *frame, size_t frame_samples)
{
    ...
    waveform_push(listen, frame_wave_level(frame, frame_samples, rms), was_active);
