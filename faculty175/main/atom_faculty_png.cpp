...
static int png_bust_draw(PNGDRAW *pDraw)
{
    ...
    /* CO5300 QSPI expects the same RGB565 byte order as lcd_pack565 (BIG_ENDIAN). */
    ctx->png->getLineAsRGB565(pDraw, line, PNG_RGB565_BIG_ENDIAN, faculty175_display_bkgd_u32());
    ctx->png->getAlphaMask(pDraw, alpha, 64);
                                        ...
                                        int *out_h)
{
    ...
    if (crop_w == 64 && crop_h == 64 && src_w == 128) {
        ...
        ESP_LOGI(TAG, "PNG bust decoded %dx%d crop %dx%d@%d,%d -> %dx%d", src_w, src_h, crop_w, crop_h, crop_x,
                 crop_y, ATOM_FACULTY_BUST_W, ATOM_FACULTY_BUST_H);
    }
