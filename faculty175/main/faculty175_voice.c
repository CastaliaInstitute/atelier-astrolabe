...
bool faculty175_voice_heap_ready(const char *stage)
{
    ...
    if (free_i < VOICE_HEAP_MIN_INTERNAL_FREE || largest_i < VOICE_HEAP_MIN_INTERNAL_LARGEST) {
        FACULTY175_LOG_STAGE_W(TAG, "heap", "%s low heap internal=%u largest=%u psram=%u need=%u/%u",
...
esp_err_t faculty175_voice_stream_begin(const char *faculty_slug, const char *faculty_name, const char *history)
{
    ...
    if (!faculty175_voice_heap_ready("voice-begin")) {
        return ESP_ERR_NO_MEM;
    }
    if (s_stream.open) {
    ...
    FACULTY175_LOG_STAGE(TAG, "stream", "VAD capture started -> %s heap=%u/%u psram=%u", s_stream.spool_path,
...
esp_err_t faculty175_voice_play_mp3(const uint8_t *mp3, size_t mp3_len)
{
    ...
    while (offset < mp3_len) {
        ...
        if (!configured) {
            ...
            configured = true;
            FACULTY175_LOG_STAGE(TAG, "tts", "decoder %u Hz ch=%d", (unsigned)info.hz, info.channels);
        }
        if (info.channels == 1) {
