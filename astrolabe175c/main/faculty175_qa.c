...
static void qa_print_help(void)
{
    ...
    printf("  qa bowl [hz] [ms]  gentle resonant bowl tone\n");
    printf("  qa pa <0|1>  force speaker amplifier pin low/high\n");
    printf("  qa spkvol [0-100]  set speaker volume\n");
    ...
    printf("  qa tasks    task stack high-water marks\n");
    printf("  qa board    Waveshare module guess + I2C/flash probe\n");
    printf("  qa chime    replay boot chime\n");
...
static void qa_emit_bowl_tone(unsigned hz, unsigned ms)
{
    if (hz == 0) {
        hz = 320;
    }
    ...
    if (!faculty175_board_audio_ready()) {
        printf("qa: bowl off (pi4ioe=%s mic_probe_peak=%ld)\n",
               faculty175_board_pi4ioe_ok() ? "ok" : "fail",
               (long)faculty175_board_mic_probe_peak());
        fflush(stdout);
        return;
    ...
    static const uint8_t voice_count = 6;
    float voice_hz[6] = {};
    float voice_phase[6] = {};
    float voice_beat_phase[6] = {};
    float voice_ping_phase[6] = {};
    float voice_amp[6] = {};
    bool struck = false;
    ...
    uint32_t written_frames = 0;
    uint32_t write_fail = 0;
    const float seed_base = 0.61803398875f * 6.28318530718f;
    for (size_t v = 0; v < voice_count; ++v) {
        const float seed = seed_base * (float)(v + 1u);
        voice_phase[v] = seed;
        voice_beat_phase[v] = seed * 1.17f;
        voice_ping_phase[v] = seed * 1.43f;
    }

    printf("qa: bowl begin hz=%u ms=%u muted=0\n", hz, ms);
    fflush(stdout);
    faculty175_audio_set_speaker_mute(false);
    ...
    while (written_frames < total_frames) {
        ...
        for (uint32_t i = 0; i < chunk; ++i) {
            const uint32_t t = written_frames + i;
            const float strike_env = (float)t < 0.10f * (float)FACULTY175_AUDIO_RATE
                                         ? (float)t / (0.10f * (float)FACULTY175_AUDIO_RATE)
                                         : 1.f;
            float sample_f = 0.f;
            if (!struck) {
                voice_hz[0] = (float)hz;
                voice_amp[0] = 1.05f;
                voice_phase[0] = (float)((esp_random() % 10000u) * 0.0006283185307f);
                voice_beat_phase[0] = (float)((esp_random() % 10000u) * 0.0006283185307f);
                voice_ping_phase[0] = (float)((esp_random() % 10000u) * 0.0006283185307f);
                struck = true;
            }
            for (size_t v = 0; v < voice_count; ++v) {
                float amp = voice_amp[v];
                if (amp < 0.0008f) {
                    voice_amp[v] = 0.f;
                    continue;
                }
                amp *= 0.999993f;
                voice_amp[v] = amp;
                float ph = voice_phase[v];
                float beat_ph = voice_beat_phase[v];
                float ping_ph = voice_ping_phase[v];
                const float vhz = voice_hz[v];
                ph += 6.28318530718f * vhz / (float)FACULTY175_AUDIO_RATE;
                beat_ph += 6.28318530718f * vhz * 1.0065f / (float)FACULTY175_AUDIO_RATE;
                ping_ph += 6.28318530718f * vhz * 2.72f / (float)FACULTY175_AUDIO_RATE;
                if (ph > 6.28318530718f) {
                    ph -= 6.28318530718f;
                }
                if (beat_ph > 6.28318530718f) {
                    beat_ph -= 6.28318530718f;
                }
                if (ping_ph > 6.28318530718f) {
                    ping_ph -= 6.28318530718f;
                }
                voice_phase[v] = ph;
                voice_beat_phase[v] = beat_ph;
                voice_ping_phase[v] = ping_ph;
                const float bright = powf(amp, 1.75f);
                sample_f += amp * (0.72f * sinf(ph) + 0.52f * sinf(beat_ph)) +
                            bright * (0.18f * sinf(ph * 2.03f) + 0.13f * sinf(ping_ph) +
                                      0.07f * sinf(ph * 3.91f));
            }
            sample_f *= 0.24f * strike_env;
            sample_f = sample_f / (1.f + fabsf(sample_f) * 0.32f);
    ...
    printf("qa: bowl end frames=%lu write_fail=%lu\n",
           (unsigned long)written_frames,
           (unsigned long)write_fail);
    fflush(stdout);
    if (s_bind.listen != NULL) {
...
static void qa_emit_board(void)
{
    ...
    printf("qa: board mac=%02x:%02x:%02x:%02x:%02x:%02x i2c axp=%d tca9554=%d es7210=%d es8311=%d touch=%d cst9217=%d cst816=%d\n",
           id->mac[0],
           id->mac[1],
           id->mac[2],
           id->mac[3],
...
bool faculty175_qa_handle(const char *line)
{
    ...
    if (strncasecmp(sub, "pa ", 3) == 0 || strcasecmp(sub, "pa") == 0) {
        const char *arg = sub + 2;
        while (*arg == ' ') {
            ++arg;
        }
        const uint32_t level = (uint32_t)strtoul(arg, NULL, 10);
        faculty175_audio_set_speaker_pa_level(level != 0u);
        printf("qa: pa level=%u\n", level ? 1u : 0u);
        fflush(stdout);
        return true;
    }
    if (strncasecmp(sub, "spkvol", 6) == 0) {
        const char *arg = sub + 6;
        while (*arg == ' ') {
            ++arg;
        }
        uint32_t level = (uint32_t)strtoul(arg, NULL, 10);
        if (level > 100u) {
            level = 100u;
        }
    ...
    if (strcasecmp(sub, "tasks") == 0) {
        if (s_bind.emit_tasks != NULL) {
            s_bind.emit_tasks();
        } else {
            printf("qa: tasks unavailable\n");
            fflush(stdout);
        }
        return true;
    }
    if (strcasecmp(sub, "chime") == 0) {
        faculty175_audio_set_speaker_mute(false);
        faculty175_audio_set_speaker_volume(100u);
        const esp_err_t err = faculty175_board_play_boot_chime();
        printf("qa: chime %s\n", esp_err_to_name(err));
        fflush(stdout);
        return true;
    }
