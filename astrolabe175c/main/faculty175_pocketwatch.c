...
bool faculty175_pocketwatch_draw_background(void)
{
    ...
    if (!s_bg_override_checked) {
        s_bg_override_checked = true;
        if (faculty175_storage_init() == ESP_OK) {
            char path[192];
            if (faculty175_storage_media_resolve_path("backgrounds/pocketwatch_default.rgb565", path, sizeof(path))) {
                ...
                if (f != NULL) {
                    ...
                    if (pixels != NULL && fread(pixels, 1, expected, f) == expected) {
                        s_bg_override_pixels = pixels;
                        ESP_LOGI(TAG, "background override loaded from FAT");
                    } else {
                        free(pixels);
                        ESP_LOGW(TAG, "background override read failed");
                    }
                    fclose(f);
                }
            }
    ...
    if (s_bg_override_pixels != NULL) {
        faculty175_display_draw_rgb565(s_bg_override_pixels, 0, 0, FACULTY175_LCD_W, FACULTY175_LCD_H);
        return true;
    }
...
bool faculty175_pocketwatch_handle(const char *line)
{
    ...
    if (*sub == '\0' || strcasecmp(sub, "status") == 0 || strcasecmp(sub, "bg") == 0) {
        printf("watch: background=%s source=%s\n",
               faculty175_pocketwatch_background_enabled() ? "on" : "off",
               s_bg_override_pixels != NULL ? "usbflash" : "flash");
        fflush(stdout);
        return true;
    }
