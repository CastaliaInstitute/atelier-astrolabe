...
- [~] **P2** **Tibetan bowl face** — new `ClockFace`: **drag finger along the screen circumference** (polar hit-test on outer ring) to excite singing-bowl tones; map touch angle → pan and drag speed → amplitude/decay. Synthesize fundamentals + harmonics via I2S (`pm_speaker` / ES8311). **Swipe up/down** cycles **bowl presets**. Visual: bowl graphic + ripple + rim finger. Tangential rim drags do not change face (horizontal swipe suppressed).
