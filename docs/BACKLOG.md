...
- [~] **P1** **Synastry face** — Issue: [#32](https://github.com/CastaliaInstitute/astrolabe/issues/32), branch `feature/32-synastry-clock-face`; PR creation pending cloud branch-tool resolution. New `ClockFace::Synastry` with NVS chart profiles (`pm_chart_profiles`), demo seed profiles, swipe up/down target cycling, dual-wheel/aspect UI, and PWR/BOOT voice prompts. Hardware QA on `integration` required before promotion.
...
- [ ] **P1** **Faculty face** — new `ClockFace` for **ask-faculty** conversations. **Flow:** **STT** (PWR) → router infers **`facultySlug`** when needed → **`ask-faculty`** (conversation **history on Castalia**, not stored on watch) → **TTS** reply. **On-device only:** NVS **recent faculty slugs** (speaker list) + **`faculty-bust`** cache per slug (`pm_faculty_bust`); **swipe up/down** scrolls **speakers** (bust + active slug), not message history. **Server:** commonplace logs turns; edge fn assembles prompt history per `facultySlug` + user. Parity with Android [`GlowScreen`](https://github.com/CastaliaInstitute/mynah/blob/main/android/app/src/main/java/institute/castalia/mynah/ui/GlowScreen.kt). See [pocketwatch.md § Backend](pocketwatch.md#backend-reuse). **Issue:** [Actions: Create backlog issue](https://github.com/CastaliaInstitute/astrolabe/actions/workflows/create-backlog-issue.yml) (`faculty-face`) or `./scripts/create-faculty-face-issue.sh` ([`scripts/issue-bodies/faculty-face.md`](../scripts/issue-bodies/faculty-face.md)).
