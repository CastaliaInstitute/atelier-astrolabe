...
{
    if (id->cst816 && id->flash_mb == 16) {
        return FACULTY175_GUESS_185B;
    }
...
{
    switch (guess) {
        case FACULTY175_GUESS_185B:
            return "ESP32-S3-Touch-LCD-1.85B";
...
void faculty175_board_log_identity(void)
{
    ...
    s_id.cst816 = i2c_has_device(0x15);
    ...
    ESP_LOGI(TAG,
             ...
             (unsigned)cfg_mb);
    ESP_LOGI(TAG,
             "I2C AXP2101=%d TCA9554=%d ES7210=%d ES8311=%d CST9217=%d CST816=%d",
             s_id.axp2101,
             s_id.tca9554,
             s_id.es7210,
             s_id.es8311,
    ...
    } else if (s_id.guess == FACULTY175_GUESS_185B) {
        FACULTY175_LOG_STAGE(TAG, "board", "16 MB flash + CST816S → 1.85B; ST77916 QSPI LCD + PWM backlight");
