...
static bool bust_cache_init(void)
{
    ...
    if (esp_spiffs_info("storage", &total, &used) == ESP_OK) {
        ESP_LOGI(TAG, "bust SPIFFS ok %u/%u KiB", (unsigned)(used / 1024), (unsigned)(total / 1024));
    }
...
static bool bust_cache_path(const char *slug, char *path, size_t cap)
{
    ...
    const int n = snprintf(path, cap, BUST_CACHE_BASE "/%s-%s-right.png", BUST_CACHE_VERSION, slug);
    return n > 0 && (size_t)n < cap;
}
...
static bool bust_load_from_seed(const char *slug, uint8_t **bytes, size_t *len, const char **source_out)
{
    if (slug == NULL || bytes == NULL || len == NULL || !bust_cache_init()) {
        return false;
    }
    const char *fallback_path = NULL;
    const char *source = NULL;
    const char *sd_relative = NULL;
    char seed_path[160];

    if (strcmp(slug, "a.plato") == 0 || strcmp(slug, "plato") == 0) {
        fallback_path = BUST_CACHE_BASE "/v5-a.plato-right.png";
        sd_relative = "bust_cache/v5-a.plato-right.png";
        source = "seed-plato";
    } else {
        fallback_path = BUST_CACHE_BASE "/v5-a.darwin-right.png";
        sd_relative = "bust_cache/v5-a.darwin-right.png";
        source = strcmp(slug, "a.darwin") == 0 || strcmp(slug, "darwin") == 0 ? "seed-darwin" : "seed-darwin-fallback";
    }
    if (!faculty175_usb_resolve_asset_path(sd_relative, fallback_path, seed_path, sizeof(seed_path))) {
        return false;
    }
    ...
    if (size <= 0 || (size_t)size > BUST_IMAGE_MAX_BYTES || fseek(f, 0, SEEK_SET) != 0) {
        fclose(f);
        return false;
    }
...
{
    if (slug == NULL || slug[0] == '\0' || bytes == NULL || len == 0 || len > BUST_IMAGE_MAX_BYTES) {
        return false;
    }
    char path[80];
    if (!bust_cache_path(slug, path, sizeof(path)) || !bust_cache_init()) {
    ...
    if (f == NULL) {
        ESP_LOGW(TAG, "bust flash save open failed %s", slug);
        return false;
    }
    ...
    if (wr != len) {
        (void)remove(path);
        ESP_LOGW(TAG, "bust flash save short %s", slug);
        return false;
    }
    ESP_LOGI(TAG, "bust flash cached %s (%u B)", slug, (unsigned)len);
    return true;
}

static bool fetch_supabase_storage_right_bust(const char *slug, uint8_t **bytes, size_t *len)
{
    if (slug == NULL || slug[0] == '\0' || bytes == NULL || len == NULL || strlen(MYNAH_SUPABASE_URL) == 0) {
        return false;
    }
    ...
    for (int si = 0; si < slug_n; ++si) {
        for (size_t li = 0; li < sizeof(kLeaves) / sizeof(kLeaves[0]); ++li) {
            ...
            if (fetch_bust_url(url, "storage-right", bytes, len, 0)) {
                return true;
            }
        }
    }
...
static bool fetch_bust_bytes_network(const char *slug, uint8_t **bytes, size_t *len, const char **source_out)
{
    ...
    if (fetch_supabase_storage_right_bust(slug, bytes, len)) {
        if (source_out != NULL) {
...
static bool fetch_and_decode_bust(const char *slug)
{
    ...
    if (from_flash) {
        ...
        if (!bust_load_from_seed(slug, &image, &image_len, &source)) {
