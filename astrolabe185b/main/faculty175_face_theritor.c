...
void faculty175_face_theritor_set_state(faculty175_ui_state_t state)
{
    ...
    if (state == FACULTY175_UI_THINK) {
        s_expression = THERITOR_EXPRESSION_THINKING;
    } else if (state == FACULTY175_UI_ERROR) {
        s_expression = THERITOR_EXPRESSION_CONCERNED;
    }
}

void faculty175_face_theritor_set_context(const char *respondent, const char *mode)
{
    if (respondent != NULL && respondent[0] != '\0') {
...
static void draw_expression(int cx, int cy, uint16_t ink, uint16_t accent)
{
    ...
    } else if (s_expression == THERITOR_EXPRESSION_WARM ||
               s_expression == THERITOR_EXPRESSION_RELIEVED) {
        for (int x = -34; x <= 34; ++x) {
