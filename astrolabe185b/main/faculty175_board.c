...
#include "astrolabe185b_st77916_init.h"

static const char *TAG = "faculty_board";

/* Waveshare ESP32-S3-Touch-LCD-1.85B — ST77916 360x360 QSPI LCD. */
#define FACULTY175_LCD_HOST SPI2_HOST
#define FACULTY175_LCD_PIN_CS GPIO_NUM_21
#define FACULTY175_LCD_PIN_PCLK GPIO_NUM_40
#define FACULTY175_LCD_PIN_DATA0 GPIO_NUM_46
#define FACULTY175_LCD_PIN_DATA1 GPIO_NUM_45
#define FACULTY175_LCD_PIN_DATA2 GPIO_NUM_42
#define FACULTY175_LCD_PIN_DATA3 GPIO_NUM_41
#define FACULTY175_LCD_PIN_RST GPIO_NUM_3
#define FACULTY175_LCD_PIN_BL GPIO_NUM_5

#define FACULTY175_LCD_PANEL_GAP_X 0
#define FACULTY175_LCD_PANEL_GAP_Y 0

#define FACULTY175_I2C_PORT I2C_NUM_0
#define FACULTY175_I2S_PORT I2S_NUM_0

#define FACULTY175_AUDIO_I2C_SDA GPIO_NUM_11
#define FACULTY175_AUDIO_I2C_SCL GPIO_NUM_10
#define FACULTY175_I2S_MCLK GPIO_NUM_2
#define FACULTY175_I2S_BCK GPIO_NUM_48
#define FACULTY175_I2S_WS GPIO_NUM_38
#define FACULTY175_I2S_DOUT GPIO_NUM_47
#define FACULTY175_I2S_DIN GPIO_NUM_39
#define FACULTY175_PA_GPIO GPIO_NUM_9
...
/** Scaled watch geometry for the 360x360 LCD. */
#define FACULTY175_PANEL_CX (FACULTY175_LCD_W / 2)
#define FACULTY175_PANEL_CY (FACULTY175_LCD_H / 2)
#define FACULTY175_BEZEL_OUTER_R (FACULTY175_LCD_W / 2 - 6)
#define FACULTY175_BEZEL_INNER_R (FACULTY175_LCD_W / 2 - 7)
#define FACULTY175_NAME_ARC_R (FACULTY175_LCD_W / 2 - 12)
...
static esp_err_t faculty175_backlight_init(void)
{
    ...
    const ledc_timer_config_t timer_cfg = {
        ...
        .timer_num = ASTROLABE185B_LCD_LEDC_TIMER,
        .freq_hz = 5000,
        .clk_cfg = LEDC_AUTO_CLK,
    };
    const ledc_channel_config_t channel_cfg = {
        .gpio_num = FACULTY175_LCD_PIN_BL,
        .speed_mode = LEDC_LOW_SPEED_MODE,
        .channel = ASTROLABE185B_LCD_LEDC_CH,
        .intr_type = LEDC_INTR_DISABLE,
        .timer_sel = ASTROLABE185B_LCD_LEDC_TIMER,
        .duty = 0,
        .hpoint = 0,
    };
...
static esp_err_t faculty175_lcd_init(void)
{
    const spi_bus_config_t bus_cfg = {
        .data0_io_num = FACULTY175_LCD_PIN_DATA0,
        .data1_io_num = FACULTY175_LCD_PIN_DATA1,
        .sclk_io_num = FACULTY175_LCD_PIN_PCLK,
        .data2_io_num = FACULTY175_LCD_PIN_DATA2,
        .data3_io_num = FACULTY175_LCD_PIN_DATA3,
        .data4_io_num = -1,
        .data5_io_num = -1,
        .data6_io_num = -1,
        .data7_io_num = -1,
    ...
    esp_lcd_panel_io_spi_config_t io_cfg = {
        .cs_gpio_num = FACULTY175_LCD_PIN_CS,
        .dc_gpio_num = -1,
        .spi_mode = 0,
        .pclk_hz = 3 * 1000 * 1000,
    ...
    ESP_RETURN_ON_ERROR(
        esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)FACULTY175_LCD_HOST, &io_cfg, &s_panel_io), TAG, "lcd io probe");

    uint8_t register_data[4] = {0};
    uint32_t lcd_cmd = ((uint32_t)ASTROLABE185B_LCD_OPCODE_READ_CMD << 24) | (0x04u << 8);
    esp_err_t read_err = esp_lcd_panel_io_rx_param(s_panel_io, lcd_cmd, register_data, sizeof(register_data));
    if (read_err != ESP_OK) {
        ESP_LOGW(TAG, "ST77916 ID read failed: %s", esp_err_to_name(read_err));
    } else {
        ESP_LOGI(TAG, "ST77916 ID reg04: %02x %02x %02x %02x",
    ...
    if (register_data[0] == 0x00 && register_data[1] == 0x7F && register_data[2] == 0x7F && register_data[3] == 0x7F) {
        vendor_cfg.init_cmds = s_st77916_init_version_1;
        vendor_cfg.init_cmds_size = sizeof(s_st77916_init_version_1) / sizeof(s_st77916_init_version_1[0]);
    } else {
        vendor_cfg.init_cmds = s_st77916_init_version_2;
        vendor_cfg.init_cmds_size = sizeof(s_st77916_init_version_2) / sizeof(s_st77916_init_version_2[0]);
    }
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_st77916(s_panel_io, &panel_cfg, &s_panel), TAG, "lcd panel");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_reset(s_panel), TAG, "lcd reset");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_init(s_panel), TAG, "lcd init");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_swap_xy(s_panel, true), TAG, "lcd swap_xy");
    ...
    ESP_RETURN_ON_ERROR(faculty175_backlight_init(), TAG, "backlight");
    ...
    ESP_LOGI(TAG, "ST77916 360x360 init ok (1.85B)");
    return ESP_OK;
}
...
void faculty175_board_set_backlight(uint8_t percent)
{
    ...
    (void)ledc_set_duty(LEDC_LOW_SPEED_MODE, ASTROLABE185B_LCD_LEDC_CH, duty);
    (void)ledc_update_duty(LEDC_LOW_SPEED_MODE, ASTROLABE185B_LCD_LEDC_CH);
}
...
static void draw_bezel_nav(void)
{
    ...
    if (face != NULL && face->id == FACULTY175_FACE_MOON) {
        return;
    }
