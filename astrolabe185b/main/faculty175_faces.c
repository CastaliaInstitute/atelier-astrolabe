...
static const faculty175_face_desc_t k_faces[] = {
    ...
    { FACULTY175_FACE_MOON, "moon", "Moon", FACULTY175_FACE_CAT_HOME | FACULTY175_FACE_CAT_ORACLE, true, true, 0 },
    ...
    { FACULTY175_FACE_THERITOR, "theritor", "Theritor", FACULTY175_FACE_CAT_COMMONPLACE, true, true, 15 },
};
...
{
    return id == FACULTY175_FACE_FACULTY || id == k_default_home_face;
}
...
esp_err_t faculty175_faces_init(void)
{
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        err = nvs_set_str(nvs, FACES_NVS_CURRENT, k_faces[k_default_home_face].slug);
        current[0] = '\0';
    }
    uint8_t schema = 0;
    ...
    if (err == ESP_OK && schema < FACES_CONFIG_RESET_SCHEMA_VERSION) {
        ...
        if (err == ESP_OK) {
            err = nvs_set_str(nvs, FACES_NVS_CURRENT, k_faces[k_default_home_face].slug);
            if (err == ESP_OK) {
                strncpy(current, k_faces[k_default_home_face].slug, sizeof(current) - 1u);
    ...
    s_current = k_default_home_face;
    if (strcmp(current, k_faces[k_default_home_face].slug) != 0) {
        nvs = 0;
...
esp_err_t faculty175_faces_set_navigation_enabled(faculty175_face_id_t id, bool enabled)
{
    ...
    if (err == ESP_OK && !enabled && s_current == id) {
        const faculty175_face_desc_t *next = faculty175_faces_nav_at(0);
        s_current = next != NULL ? next->id : k_default_home_face;
    }
    return err;
}
