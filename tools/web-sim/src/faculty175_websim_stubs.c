...
{
    return "websim asset unavailable";
}
...
esp_err_t faculty175_wifi_settings_add_known(const char *ssid, const char *pass, bool make_primary) { (void)ssid; (void)pass; (void)make_primary; return ESP_OK; }
...
const char *faculty175_wifi_settings_ap_qr_payload(void) { return faculty175_wifi_settings_qr_payload(); }
const char *faculty175_wifi_settings_page_qr_payload(void) { return "http://localhost:8088"; }
...
void faculty175_wifi_monitor_record_disconnected(const char *ssid, const uint8_t bssid[6], int reason, int rssi) { (void)ssid; (void)bssid; (void)reason; (void)rssi; }
...
size_t faculty175_wifi_monitor_copy(faculty175_wifi_incident_t *out, size_t cap) { (void)out; (void)cap; return 0; }
size_t faculty175_wifi_monitor_count(void) { return 0; }
bool faculty175_wifi_monitor_get_newest(size_t offset, faculty175_wifi_incident_t *out) { (void)offset; (void)out; return false; }
void faculty175_wifi_monitor_clear(void) {}

bool faculty175_wifi_lab_is_face(faculty175_face_id_t id)
{
    return id == FACULTY175_FACE_WSCAN ||
           id == FACULTY175_FACE_DEAUTH ||
           id == FACULTY175_FACE_EVILTWIN ||
...
bool faculty175_wifi_lab_tap(faculty175_face_id_t id) { (void)id; return false; }
bool faculty175_wifi_lab_cycle_target(faculty175_face_id_t id, int delta) { (void)id; (void)delta; return false; }
void faculty175_wifi_lab_get_state(faculty175_wifi_lab_state_t *out)
{
    ...
    snprintf(out->status, sizeof(out->status), "websim Wi-Fi demo");
    snprintf(out->detail, sizeof(out->detail), "ESP Wi-Fi unavailable in browser");
}
...
esp_err_t faculty175_wifi_lab_export_pcap(void) { return ESP_FAIL; }
...
esp_err_t faculty175_location_settings_save(double lat_deg, double lon_deg, const char *source) { (void)lat_deg; (void)lon_deg; (void)source; return ESP_OK; }
bool faculty175_location_settings_valid(double lat_deg, double lon_deg) { return lat_deg >= -90.0 && lat_deg <= 90.0 && lon_deg >= -180.0 && lon_deg <= 180.0; }
...
bool faculty175_apocalypso_current(faculty175_apocalypso_status_t *out)
{
    ...
    for (size_t i = 0; i < FACULTY175_APOCALYPSO_AXIS_COUNT; ++i) {
        out->value[i] = (float)((i * 17u) % 100u);
                                           ...
                                           size_t slug_cap)
{
    ...
    if (slug_out && slug_cap > 0) {
        snprintf(slug_out, slug_cap, "websim");
    }
    return false;
}
