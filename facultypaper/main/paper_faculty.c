...
static const char *TAG = "paper_faculty";
...
static uint8_t bust_luma_rgb(const uint8_t *rgb)
{
    return (uint8_t)(((uint16_t)rgb[0] * 77u + (uint16_t)rgb[1] * 150u + (uint16_t)rgb[2] * 29u) >> 8);
}

static void bust_stylize_stencil(const uint8_t *rgb, const uint8_t *opaque, uint16_t *out, int w, int h)
{
    const uint16_t ink = paper_display_rgb888(0, 72, 220);
    const uint16_t white = paper_display_rgb888(255, 255, 255);
    const size_t count = (size_t)w * (size_t)h;
    uint8_t *luma = (uint8_t *)heap_caps_malloc(count, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    uint8_t *blur = (uint8_t *)heap_caps_malloc(count, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    uint8_t *mask = (uint8_t *)heap_caps_malloc(count, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (luma == NULL) {
        luma = (uint8_t *)malloc(count);
    }
    if (blur == NULL) {
        blur = (uint8_t *)malloc(count);
    }
    if (mask == NULL) {
        mask = (uint8_t *)malloc(count);
    }
    if (luma == NULL || blur == NULL || mask == NULL) {
        for (int i = 0; i < w * h; ++i) {
            out[i] = white;
        }
        free(luma);
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            const int i = y * w + x;
            if (opaque != NULL && opaque[i] == 0) {
                luma[i] = 255;
                continue;
            }
            const uint8_t l = bust_luma_rgb(rgb + (size_t)i * 3);
            luma[i] = l;
            if (l < lo) {
                lo = l;
            }
    ...
    const int range = hi > lo ? (int)hi - (int)lo : 1;
    for (size_t i = 0; i < count; ++i) {
        int v = ((int)luma[i] - (int)lo) * 255 / range;
        if (v < 0) {
            v = 0;
        } else if (v > 255) {
            v = 255;
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            const int gradient = abs((int)luma[y * w + xr] - (int)luma[y * w + xl]) +
                                 abs((int)luma[yd * w + x] - (int)luma[yu * w + x]);
            const bool shadow_mass = blur[i] < 116;
            const bool contour = gradient > 44 && luma[i] < 225;
            mask[i] = (shadow_mass || contour) ? 1 : 0;
        }
    }
        ...
        for (int x = 0; x < w; ++x) {
            ...
            for (int yy = y - 1; yy <= y + 1; ++yy) {
                ...
                for (int xx = x - 1; xx <= x + 1; ++xx) {
                    ...
                    neighbors += mask[yy * w + xx] != 0;
                }
            }
            const bool keep = mask[i] ? neighbors >= 2 : neighbors >= 6;
            out[i] = keep ? ink : white;
        }
    }
    free(luma);
    free(blur);
...
static paper_faculty_bust_status_t s_status = PAPER_FACULTY_BUST_IDLE;
EXT_RAM_BSS_ATTR static uint16_t s_bust_pixels[PAPER_FACULTY_BUST_W * PAPER_FACULTY_BUST_H];
EXT_RAM_BSS_ATTR static uint8_t s_bust_opaque[PAPER_FACULTY_BUST_W * PAPER_FACULTY_BUST_H];
static int s_bust_draw_w;
static int s_bust_draw_h;
static paper_faculty_ui_notify_fn s_ui_notify;
...
static bool build_castalia_bust_url(const char *slug, char *url, size_t cap)
{
    ...
    const int n = snprintf(url, cap, "%s/api/faculty-bust/?faculty=%s&w=%d&h=%d&q=60", base, slug,
                           PAPER_FACULTY_BUST_W, PAPER_FACULTY_BUST_H);
    return n > 0 && (size_t)n < cap;
}
...
static bool build_supabase_bust_url(const char *slug, char *url, size_t cap)
{
    ...
    const int n = snprintf(url, cap,
                           "%s/functions/v1/faculty-bust?faculty=%s&w=%d&h=%d&q=60&resize=cover&format=png&view=line&v=line1",
                           base, slug, PAPER_FACULTY_BUST_W, PAPER_FACULTY_BUST_H);
    return n > 0 && (size_t)n < cap;
}
...
{
    return paper_faculty_png_decode(png, png_len, out, opaque, out_w, out_h);
}
...
static bool decode_jpeg_rgb565(const uint8_t *jpeg, size_t jpeg_len, uint16_t *out, uint8_t *opaque, int *out_w, int *out_h)
{
    ...
    const size_t bust_px = (size_t)PAPER_FACULTY_BUST_W * (size_t)PAPER_FACULTY_BUST_H;
    ...
    for (int y = 0; y < PAPER_FACULTY_BUST_H; ++y) {
        const int sy = (PAPER_FACULTY_BUST_H == 1) ? 0 : (y * (decoded_h - 1)) / (PAPER_FACULTY_BUST_H - 1);
        for (int x = 0; x < PAPER_FACULTY_BUST_W; ++x) {
            const int sx = (PAPER_FACULTY_BUST_W == 1) ? 0 : (x * (decoded_w - 1)) / (PAPER_FACULTY_BUST_W - 1);
            const uint8_t *px = native + ((size_t)sy * (size_t)decoded_w + (size_t)sx) * 3;
            const size_t di = ((size_t)y * (size_t)PAPER_FACULTY_BUST_W + (size_t)x) * 3;
            bust_rgb[di + 0] = px[0];
            bust_rgb[di + 1] = px[1];
            bust_rgb[di + 2] = px[2];
        }
    ...
    apply_rgb888_chroma_mask(opaque, PAPER_FACULTY_BUST_W, PAPER_FACULTY_BUST_H, bust_rgb, 3);

    bust_stylize_stencil(bust_rgb, opaque, out, PAPER_FACULTY_BUST_W, PAPER_FACULTY_BUST_H);
...
static bool fetch_and_decode_bust(const char *slug)
{
    ...
    uint16_t *scratch =
        heap_caps_malloc(PAPER_FACULTY_BUST_W * PAPER_FACULTY_BUST_H * sizeof(uint16_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    uint8_t *opaque =
        heap_caps_malloc(PAPER_FACULTY_BUST_W * PAPER_FACULTY_BUST_H, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (scratch == NULL) {
        scratch = malloc(PAPER_FACULTY_BUST_W * PAPER_FACULTY_BUST_H * sizeof(uint16_t));
    }
    if (opaque == NULL) {
        opaque = malloc(PAPER_FACULTY_BUST_W * PAPER_FACULTY_BUST_H);
    }
    ...
    if (xSemaphoreTake(s_bust_lock, pdMS_TO_TICKS(2000)) == pdTRUE) {
        ...
        s_status = PAPER_FACULTY_BUST_READY;
    ...
    PAPER_LOG_STAGE(TAG, "faculty", "bust ready %s via %s (%dx%d)", slug, source != NULL ? source : "?", draw_w, draw_h);
    return true;
}
...
static void bust_worker_task(void *arg)
{
    ...
    while (true) {
        ...
        if (xSemaphoreTake(s_bust_lock, pdMS_TO_TICKS(1000)) == pdTRUE) {
            if (s_status == PAPER_FACULTY_BUST_READY && strcmp(s_loaded_slug, slug) == 0) {
                xSemaphoreGive(s_bust_lock);
                continue;
            }
            s_status = PAPER_FACULTY_BUST_LOADING;
            xSemaphoreGive(s_bust_lock);
        }

        PAPER_LOG_STAGE(TAG, "faculty", "bust loading %s", slug);
        ...
        if (!ok) {
            if (xSemaphoreTake(s_bust_lock, pdMS_TO_TICKS(1000)) == pdTRUE) {
                s_status = PAPER_FACULTY_BUST_ERROR;
                xSemaphoreGive(s_bust_lock);
            }
            ESP_LOGW(TAG, "bust fetch failed for %s", slug);
            PAPER_LOG_STAGE_W(TAG, "faculty", "bust fetch failed for %s", slug);
        }
...
void paper_faculty_request_bust(const char *slug)
{
    ...
    if (xSemaphoreTake(s_bust_lock, pdMS_TO_TICKS(200)) == pdTRUE) {
        if (s_status == PAPER_FACULTY_BUST_LOADING && strcmp(s_req_slug, slug) == 0) {
            xSemaphoreGive(s_bust_lock);
            return;
        }
        if (s_status == PAPER_FACULTY_BUST_READY && strcmp(s_loaded_slug, slug) == 0) {
...
{
    paper_faculty_bust_status_t status = PAPER_FACULTY_BUST_IDLE;
...
bool paper_faculty_draw_bust(int x, int y)
{
    ...
    bool drew = false;
    if (xSemaphoreTake(s_bust_lock, pdMS_TO_TICKS(100)) == pdTRUE) {
        if (s_status == PAPER_FACULTY_BUST_READY && s_bust_draw_w > 0 && s_bust_draw_h > 0) {
            paper_display_blit_rgb565_masked(s_bust_pixels, s_bust_opaque, x, y, s_bust_draw_w, s_bust_draw_h);
            drew = true;
        }
        xSemaphoreGive(s_bust_lock);
    }
