...
static const char *TAG = "paper_voice";
                                   ...
                                   int *out_status)
{
    ...
    if (status < 200 || status >= 300) {
        ...
        if (err_read > 0) {
            err_body[err_read] = '\0';
            char clipped[96];
            paper_log_clip(clipped, sizeof(clipped), err_body, 80);
            PAPER_LOG_STAGE_W(TAG, "pipeline", "HTTP %d err=%s", status, clipped);
        } else {
            PAPER_LOG_STAGE_W(TAG, "pipeline", "HTTP %d (no body)", status);
        }
        esp_http_client_close(client);
        esp_http_client_cleanup(client);
        return ESP_FAIL;
                              ...
                              paper_voice_result_t *result)
{
    ...
    const char *active_slug =
        (faculty_slug != NULL && faculty_slug[0] != '\0') ? faculty_slug : ASTROLABE_FACULTY_PAPER_DEFAULT_FACULTY_SLUG;
    const char *active_name =
        (faculty_name != NULL && faculty_name[0] != '\0') ? faculty_name : ASTROLABE_FACULTY_PAPER_DEFAULT_FACULTY_NAME;
    ...
    snprintf(sys, sizeof(sys),
             ...
             ASTROLABE_FACULTY_PAPER_SYSTEM_INSTRUCTION, active_name, active_slug,
    ...
    const int body_len = snprintf(body, body_cap,
                                  ...
                                  ASTROLABE_FACULTY_PAPER_FACE_NAME, esc_slug, esc_name, esc_sys, audio_b64);
    free(audio_b64);
    free(esc_sys);
    free(esc_slug);
    free(esc_name);
    ...
    const uint32_t t0 = paper_log_ms();
    PAPER_LOG_STAGE(TAG, "pipeline", "POST %s pcm=%uB face=%s faculty=%s (%s)",
                   url, (unsigned)pcm_len, ASTROLABE_FACULTY_PAPER_FACE_NAME, active_name, active_slug);
    ...
    free(body);
    const uint32_t http_ms = paper_log_ms() - t0;
    PAPER_LOG_STAGE(TAG, "pipeline", "HTTP %d body=%uB in %ums err=%s", status, (unsigned)response_len, (unsigned)http_ms,
                   esp_err_to_name(ret));
    ...
    if (response_len > 0 && response[0] != '{') {
        PAPER_LOG_STAGE(TAG, "tts", "raw MP3 response %uB (no JSON wrapper)", (unsigned)response_len);
        result->mp3 = response;
    ...
    if (result->transcript[0] != '\0') {
        paper_log_clip(clip, sizeof(clip), result->transcript, 96);
        PAPER_LOG_STAGE(TAG, "stt", "transcript=\"%s\"", clip);
    } else {
        PAPER_LOG_STAGE_W(TAG, "stt", "empty transcript");
    }
    if (result->route[0] != '\0') {
        PAPER_LOG_STAGE(TAG, "llm", "route=%s", result->route);
    }
    if (result->faculty_slug[0] != '\0' || result->faculty_name[0] != '\0') {
        PAPER_LOG_STAGE(TAG, "llm", "faculty=%s (%s)", result->faculty_name[0] ? result->faculty_name : "?",
                       result->faculty_slug[0] ? result->faculty_slug : "?");
    }
    if (result->reply[0] != '\0') {
        paper_log_clip(clip, sizeof(clip), result->reply, 96);
        PAPER_LOG_STAGE(TAG, "llm", "reply=\"%s\"", clip);
    } else {
        PAPER_LOG_STAGE_W(TAG, "llm", "empty reply text");
    }
    if (result->mp3 != NULL && result->mp3_len > 0) {
        PAPER_LOG_STAGE(TAG, "tts", "mp3=%uB embedded in JSON", (unsigned)result->mp3_len);
    } else {
        PAPER_LOG_STAGE_W(TAG, "tts", "no audioBase64 in response");
    }

    if (result->transcript[0] == '\0' && result->reply[0] == '\0' && result->mp3 == NULL) {
        PAPER_LOG_STAGE_E(TAG, "pipeline", "response missing transcript, reply, and audio");
        return ESP_FAIL;
    }
    PAPER_LOG_STAGE(TAG, "pipeline", "turn complete in %ums", (unsigned)(paper_log_ms() - t0));
    return ESP_OK;
}
...
esp_err_t paper_voice_play_mp3(const uint8_t *mp3, size_t mp3_len)
{
    if (mp3 == NULL || mp3_len < 64) {
        PAPER_LOG_STAGE_W(TAG, "tts", "play skipped - mp3 too small (%uB)", (unsigned)mp3_len);
        return ESP_ERR_INVALID_ARG;
    }

    const uint32_t t0 = paper_log_ms();
    PAPER_LOG_STAGE(TAG, "tts", "play start mp3=%uB", (unsigned)mp3_len);
    ...
    while (offset < mp3_len) {
        ...
        if (!configured) {
            ESP_ERROR_CHECK_WITHOUT_ABORT(paper_audio_set_sample_rate((uint32_t)info.hz));
            configured = true;
            PAPER_LOG_STAGE(TAG, "tts", "decoder %u Hz ch=%d", (unsigned)info.hz, info.channels);
        }
        const size_t count = (size_t)samples * (info.channels == 1 ? 1 : 2);
        ESP_ERROR_CHECK_WITHOUT_ABORT(paper_audio_write_pcm(pcm, count, 1000));
        vTaskDelay(1);
    }

    free(pcm);
    ESP_ERROR_CHECK_WITHOUT_ABORT(paper_audio_set_sample_rate(PAPER_AUDIO_RATE));
    PAPER_LOG_STAGE(TAG, "tts", "play done in %ums", (unsigned)(paper_log_ms() - t0));
    return ESP_OK;
}
