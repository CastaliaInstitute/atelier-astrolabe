...
static esp_err_t stream_pcm_file(astrolabe_audio_pipeline_t *p, const utterance_t *utt, voice_result_t *result)
{
    ...
    stream_response_ctx_t response_ctx = {
        .result = result,
        .err = ESP_OK,
    };
    if (result != NULL) {
        response_ctx.done = xSemaphoreCreateBinary();
        if (response_ctx.done == NULL) {
        ...
        (void)esp_websocket_register_events(client, WEBSOCKET_EVENT_ANY, stream_response_event, &response_ctx);
    }
    ...
    if (err == ESP_OK && result != NULL) {
        ...
        if (xSemaphoreTake(response_ctx.done, pdMS_TO_TICKS(timeout_ms)) != pdTRUE) {
            err = ESP_ERR_TIMEOUT;
        } else if (response_ctx.err != ESP_OK) {
            err = response_ctx.err;
        } else if (!response_ctx.response_done) {
            err = ESP_FAIL;
        } else if (utt->final_segment && result->transcript[0] == '\0' && result->reply[0] == '\0' && result->mp3_len == 0) {
            err = ESP_FAIL;
        }
    }
    ...
    esp_websocket_client_destroy(client);
    if (response_ctx.done != NULL) {
        vSemaphoreDelete(response_ctx.done);
    }
    free(response_ctx.message);
