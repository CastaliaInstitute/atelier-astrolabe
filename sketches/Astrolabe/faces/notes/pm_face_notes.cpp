...
void draw_elapsed_ring(float progress, uint16_t col) {
  ...
  const int steps = static_cast<int>(progress * static_cast<float>(pm_face_scale_i(220)));
  ...
  for (int i = 0; i <= n; ++i) {
    ...
    if (have0) {
      ...
      pm_gfx->drawLine(x0, y0 + pm_face_scale_i(1), x, y + pm_face_scale_i(1), col);
      pm_gfx->drawLine(x0 + pm_face_scale_i(1), y0, x + pm_face_scale_i(1), y, col);
    }
    x0 = x;
    y0 = y;
    have0 = true;
...
void draw_input_fft(int cx, int cy, int w, int h) {
  ...
  const int gap = pm_face_scale_i(2);
...
void pm_face_notes_draw(void) {
  ...
  pm_gfx->fillRect(0, 0, LCD_WIDTH, pm_face_scale_y(54), c_panel);
  pm_face_draw_centered_line("NOTES", 10, c_accent, 2, 2);

  pm_gfx->drawCircle(LCD_WIDTH / 2, LCD_HEIGHT / 2 - pm_face_scale_i(14), pm_face_scale_i(128),
                     pm_gfx->color565(42, 56, 62));
  pm_gfx->drawCircle(LCD_WIDTH / 2, LCD_HEIGHT / 2 - pm_face_scale_i(14), pm_face_scale_i(118),
                     pm_gfx->color565(30, 42, 50));
  pm_gfx->fillCircle(LCD_WIDTH / 2, LCD_HEIGHT / 2 - pm_face_scale_i(14), pm_face_scale_i(62),
                     pm_gfx->color565(18, 28, 34));
  pm_gfx->drawCircle(LCD_WIDTH / 2, LCD_HEIGHT / 2 - pm_face_scale_i(14), pm_face_scale_i(62), c_accent);

  const int cx = LCD_WIDTH / 2;
  const int cy = LCD_HEIGHT / 2 - pm_face_scale_i(14);
  pm_gfx->fillRect(cx - pm_face_scale_i(18), cy - pm_face_scale_i(34), pm_face_scale_i(36), pm_face_scale_i(68),
                   c_ink);
  pm_gfx->fillRect(cx - pm_face_scale_i(12), cy - pm_face_scale_i(44), pm_face_scale_i(24), pm_face_scale_i(16),
                   c_gold);
  pm_gfx->drawRect(cx - pm_face_scale_i(18), cy - pm_face_scale_i(34), pm_face_scale_i(36), pm_face_scale_i(68),
                   c_accent);
  for (int y = cy - pm_face_scale_i(18); y <= cy + pm_face_scale_i(18); y += pm_face_scale_i(14)) {
    pm_gfx->drawLine(cx - pm_face_scale_i(8), y, cx + pm_face_scale_i(12), y, pm_gfx->color565(80, 94, 104));
  }
...
void pm_face_notes_draw_recording(float progress, uint32_t elapsed_ms) {
  ...
  const int cx = LCD_WIDTH / 2;
  const int cy = LCD_HEIGHT / 2 - pm_face_scale_i(10);
  pm_gfx->drawCircle(cx, cy, pm_face_scale_i(146), c_track);
  pm_gfx->drawCircle(cx, cy, pm_face_scale_i(106), pm_gfx->color565(26, 32, 42));
  pm_gfx->fillCircle(cx, cy, pm_face_scale_i(70), c_panel);

  const float pulse = 0.5f + 0.5f * sinf(static_cast<float>(elapsed_ms) * 0.010f);
  pm_gfx->fillCircle(cx - pm_face_scale_i(46), cy - pm_face_scale_i(40),
                     pm_face_scale_i(7 + static_cast<int>(pulse * 3.f)), c_red);
  pm_face_draw_centered_line("REC", cy - pm_face_scale_i(52), c_red, 2, 2);
  ...
  pm_face_draw_centered_line(time_line, cy - pm_face_scale_i(12), c_ink, 3, 3);

  pm_gfx->fillRect(cx - pm_face_scale_i(70), cy + pm_face_scale_i(38), pm_face_scale_i(140), pm_face_scale_i(62),
                   pm_gfx->color565(12, 16, 24));
  pm_gfx->drawRect(cx - pm_face_scale_i(70), cy + pm_face_scale_i(38), pm_face_scale_i(140), pm_face_scale_i(62),
                   c_track);
  draw_input_fft(cx, cy + pm_face_scale_i(69), pm_face_scale_i(122), pm_face_scale_i(46));

  pm_gfx->fillRect(cx - pm_face_scale_i(16), cy + pm_face_scale_i(114), pm_face_scale_i(32), pm_face_scale_i(44),
                   c_paper);
  pm_gfx->drawRect(cx - pm_face_scale_i(16), cy + pm_face_scale_i(114), pm_face_scale_i(32), pm_face_scale_i(44),
                   c_red);
  pm_gfx->drawLine(cx - pm_face_scale_i(8), cy + pm_face_scale_i(128), cx + pm_face_scale_i(9),
                   cy + pm_face_scale_i(128), c_track);
  pm_gfx->drawLine(cx - pm_face_scale_i(8), cy + pm_face_scale_i(140), cx + pm_face_scale_i(9),
                   cy + pm_face_scale_i(140), c_track);
