...
uint32_t pm_heap_psram_largest(void) {
  return static_cast<uint32_t>(heap_caps_get_largest_free_block(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
}
...
void pm_heap_log(const char *tag) {
  Serial.printf("heap: %s internal=%u largest=%u psram=%u psram_largest=%u\n", tag ? tag : "-",
                static_cast<unsigned>(pm_heap_internal_free()), static_cast<unsigned>(pm_heap_internal_largest()),
                static_cast<unsigned>(pm_heap_psram_free()), static_cast<unsigned>(pm_heap_psram_largest()));
}

void pm_heap_trace(const char *stage, int face) {
  Serial.printf("heap_trace: stage=%s face=%d internal=%u largest=%u psram=%u psram_largest=%u\n",
                stage ? stage : "-", face, static_cast<unsigned>(pm_heap_internal_free()),
