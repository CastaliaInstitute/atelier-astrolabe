...
static bool play_bowl_voice_streaming(void) {
  ...
  if (es8311_board_init(kToneHz) != ESP_OK) {
    ...
    Serial.println("bowl: es8311 init failed");
    return false;
  }
  if (i2s_tx_begin(kToneHz, 2) != ESP_OK) {
    ESP_LOGW(TAG, "i2s begin failed (bowl voice)");
    Serial.println("bowl: i2s begin failed");
    return false;
  }

  int16_t *buf = static_cast<int16_t *>(
      heap_caps_malloc(512u * 2u * sizeof(int16_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (!buf) {
    buf = static_cast<int16_t *>(malloc(512u * 2u * sizeof(int16_t)));
  }
  if (!buf) {
    i2s_tx_stop();
  ...
  s_bowl_stop = false;
  Serial.printf("bowl: audio start hz=%.1f brightness=%.2f\n", static_cast<double>(s_bowl.target_hz),
  ...
  while (!s_bowl_stop) {
    ...
    if (i2s_write_all(buf, frame * 2) != ESP_OK) {
      ...
      Serial.println("bowl: i2s write failed");
      return false;
    }
...
bool pm_speaker_bowl_voice_test(float hz, uint32_t hold_ms) {
  ...
  strike.rim_quality = 1.f;
  strike.center_strike = true;
  pm_speaker_bowl_voice_push(strike);
  const uint32_t start = millis();
  while (!pm_speaker_bowl_voice_active() && millis() - start < 500u) {
    delay(5);
  }
  if (!pm_speaker_bowl_voice_active()) {
    return false;
  }
  delay(hold_ms);
  return true;
