 ...
 * birth lat/lon and UTC offset using low-precision sidereal-time math; positions use the same local
 * formulas as transits, so they are suitable for watch readings but not Swiss Ephemeris replacement.
 */
typedef struct {
...
/** Compute current positions locally for `utc`, then derive natal aspects and house events. */
