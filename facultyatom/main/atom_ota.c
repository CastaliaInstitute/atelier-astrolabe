...
#include "astrolabe_faculty_atom_ota_key.h"
#include "atom_log.h"

static const char *TAG = "atom_ota";
...
static esp_err_t verify_manifest_signature(const char *canonical, const char *signature_b64)
{
    ...
    rc = mbedtls_pk_parse_public_key(&pk,
                                     (const unsigned char *)ASTROLABE_FACULTY_ATOM_OTA_PUBKEY_PEM,
                                     strlen(ASTROLABE_FACULTY_ATOM_OTA_PUBKEY_PEM) + 1);
    if (rc != 0) {
        mbedtls_pk_free(&pk);
    ...
    ATOM_LOG_STAGE(TAG, "ota", "manifest signature ok");
    return ESP_OK;
}
...
static bool ota_heap_ready(void)
{
    ...
    if (free_internal < OTA_MIN_INTERNAL_FREE || largest_internal < OTA_MIN_LARGEST_BLOCK) {
        ...
        ATOM_LOG_STAGE_W(TAG, "ota", "%s", s_ota_last);
        return false;
    }
    return true;
}
...
static esp_err_t fetch_manifest(const char *manifest_url, ota_job_t *job)
{
    ...
    int status = 0;
    if (err == ESP_OK) {
        esp_http_client_set_header(client, "User-Agent", "Astrolabe-Atom-OTA/1");
    ...
    if (err == ESP_OK) {
        ...
        if (root == NULL) {
            ...
            if (!cJSON_IsString(channel) ||
                strcmp(channel->valuestring, ASTROLABE_FACULTY_ATOM_OTA_CHANNEL) != 0) {
                ...
                err = ESP_ERR_INVALID_SIZE;
                       ...
                       strcmp(sig_alg->valuestring, ASTROLABE_FACULTY_ATOM_OTA_SIG_ALG) != 0) {
                                    ...
                                    const esp_partition_t **out_target)
{
    ...
    if (err == ESP_OK) {
        ATOM_LOG_STAGE(TAG, "ota", "install %lldB -> %s @0x%lx", content_len, target->label,
    ...
    while (err == ESP_OK) {
        ...
        if ((total % (128 * 1024)) < (size_t)n || total == (size_t)n) {
            ATOM_LOG_STAGE(TAG, "ota", "wrote %u/%lld", (unsigned)total, content_len);
        }
    ...
    if (err == ESP_OK && sha_started) {
        ...
        if (mbedtls_sha256_finish(&sha_ctx, digest) != 0) {
            ...
            if (strcasecmp(actual, expected_sha256) != 0) {
                ...
                ATOM_LOG_STAGE(TAG, "ota", "sha256 ok %s", actual);
            }
        }
    ...
    if (err == ESP_OK) {
        ...
        if (esp_ota_get_partition_description(target, &desc) == ESP_OK) {
            ATOM_LOG_STAGE(TAG, "ota", "image version=%s project=%s", desc.version, desc.project_name);
        }
...
static void ota_task(void *arg)
{
    ...
    if (err == ESP_OK) {
        ...
        ATOM_LOG_STAGE(TAG, "ota", "installed boot=%s; rebooting", part_label(target));
        vTaskDelay(pdMS_TO_TICKS(750));
        esp_restart();
    } else {
        s_ota_state = OTA_STATE_ERROR;
        ATOM_LOG_STAGE_E(TAG, "ota", "install failed: %s (%s)", esp_err_to_name(err), s_ota_last);
...
void atom_ota_maybe_start_recovery_request(void)
{
    ...
    ATOM_LOG_STAGE(TAG, "ota", "factory recovery request pending");
    esp_err_t err = start_install_task(url, NULL, 0, true);
    if (err != ESP_OK) {
        set_last("start failed: %s", esp_err_to_name(err));
        s_ota_state = OTA_STATE_ERROR;
        ATOM_LOG_STAGE_E(TAG, "ota", "%s", s_ota_last);
    }
}
