                              ...
                              atom_voice_result_t *result)
{
    ...
    const char *active_slug =
        (faculty_slug != NULL && faculty_slug[0] != '\0') ? faculty_slug : ASTROLABE_FACULTY_ATOM_DEFAULT_FACULTY_SLUG;
    const char *active_name =
        (faculty_name != NULL && faculty_name[0] != '\0') ? faculty_name : ASTROLABE_FACULTY_ATOM_DEFAULT_FACULTY_NAME;
    ...
    snprintf(sys, sizeof(sys),
             ...
             ASTROLABE_FACULTY_ATOM_SYSTEM_INSTRUCTION, active_name, active_slug,
    ...
    const int body_len = snprintf(body, body_cap,
                                  ...
                                  ASTROLABE_FACULTY_ATOM_FACE_NAME, esc_slug, esc_name, esc_sys, audio_b64);
    free(audio_b64);
    free(esc_sys);
    free(esc_slug);
    free(esc_name);
    ...
    ATOM_LOG_STAGE(TAG, "pipeline", "POST %s pcm=%uB face=%s faculty=%s (%s)",
                   url, (unsigned)pcm_len, ASTROLABE_FACULTY_ATOM_FACE_NAME, active_name, active_slug);
