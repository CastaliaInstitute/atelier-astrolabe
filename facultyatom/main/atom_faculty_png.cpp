...
static int png_bust_draw(PNGDRAW *pDraw)
{
    ...
    const uint32_t bkgd_u32 = atom_display_bkgd_u32();
    ctx->png->getLineAsRGB565(pDraw, line, PNG_RGB565_BIG_ENDIAN, bkgd_u32);
    ...
    for (int dx = 0; dx < ctx->out_w; ++dx) {
        ...
        const uint16_t fb_px = atom_display_fb_from_logical565(px);
        const uint8_t is_opaque = (px == atom_display_pack_rgb888(0, 0, 0)) ? 0 : 255;
        for (int dst_y = dst_y0; dst_y < dst_y1; ++dst_y) {
            const int out_i = dst_y * ctx->out_w + dx;
            ctx->out[out_i] = fb_px;
            ctx->opaque[out_i] = is_opaque;
        }
    }
    return 1;
}
                                        ...
                                        int *out_h)
{
    ...
    ESP_LOGI(TAG, "PNG bust decoded %dx%d -> %dx%d", src_w, src_h, ATOM_FACULTY_BUST_W, ATOM_FACULTY_BUST_H);
