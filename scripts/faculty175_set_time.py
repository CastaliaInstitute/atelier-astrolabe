...
try:
    ...
    raise SystemExit("error: pyserial is required for faculty175_set_time.py") from exc
...
def resolve_port(requested: str | None) -> str:
    if requested:
        return requested
    for env_name in ("FACULTY175_TIME_PORT", "ASTROLABE_UPLOAD_PORT", "UPLOAD_PORT", "ESPPORT", "IDF_PORT"):
    ...
    raise SystemExit("error: multiple serial ports found; pass --port or set FACULTY175_TIME_PORT")
...
def main() -> int:
    ...
    parser.add_argument("--port", help="serial port, defaults to FACULTY175_TIME_PORT/ESPPORT/11301")
    parser.add_argument("--baud", type=int, default=DEFAULT_BAUD)
    parser.add_argument("--tz", default=os.environ.get("FACULTY175_POSIX_TZ", DEFAULT_TZ))
    ...
    print(f"faculty175_set_time: synced port={port} epoch={args.epoch} tz={args.tz}")
    return 0
