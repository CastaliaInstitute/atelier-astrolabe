...
def resolve_port(port: str | None) -> str:
    if port:
        return port
    for name in (
        "FACULTY175_VOICE_PORT",
        "FACULTY175_TIME_PORT",
...
def main() -> int:
    ...
    parser.add_argument("--audio-device", default=os.environ.get("FACULTY175_MAC_AUDIO_DEVICE", "1"),
    ...
    if args.dry_run:
        ...
        print(f"faculty175_voice_validate: dry-run ok outdir={outdir} host_audio_ok={audio_ok}")
    ...
    print(f"faculty175_voice_validate: outdir={outdir}")
    print(f"faculty175_voice_validate: stt_ok={stt_ok} tts_trigger_ok={tts_ok}")
