#!/usr/bin/env bash
# Identify a USB-connected Waveshare AMOLED module before flashing faculty175 vs faculty18.
set -euo pipefail
...
if [[ "${FLASH_MB}" == "16" ]]; then
  ...
  echo "Firmware:      faculty175 (set CONFIG_ESPTOOLPY_FLASHSIZE_16MB=y in faculty175/sdkconfig)"
  ...
  echo "Firmware:      faculty175 (CONFIG_ESPTOOLPY_FLASHSIZE_32MB=y)"
else
  echo "Flash size unclear — check Waveshare silkscreen / product page"
fi

echo
echo "After flashing faculty175, confirm on-device:"
echo "  ./scripts/faculty175_build.sh -p ${PORT} monitor"
echo "  (type: qa board)"
echo
...
echo "  | 1.75C  | 32 MB | 466 CO5300   | faculty175 |"
echo "  | 1.75   | 16 MB | 466 CO5300   | faculty175 |"
