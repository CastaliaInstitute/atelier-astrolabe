#!/usr/bin/env bash
# Leave USB download mode and run faculty175 after flash (ESP32-S3 USB-JTAG).
set -euo pipefail
...
echo "faculty175_run: starting app on ${PORT}" >&2
"${IDF_PY}" -m esptool --chip esp32s3 -p "${PORT}" --before usb_reset --after hard_reset run >/dev/null
echo "faculty175_run: if the screen stays dark, tap RESET once (do not hold BOOT)." >&2
