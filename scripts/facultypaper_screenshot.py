#!/usr/bin/env python3
"""Capture FacultyPaper (M5 PaperColor) display over USB serial (screen command -> 24-bit BMP)."""
...
def capture_screenshot(
    ...
    *,
    ...
    try:
        ...
        def append_console_binary(data: bytes) -> None:
            # ESP-IDF stdout expands LF to CRLF even inside our binary BMP payload.
            # Reversing that transform preserves real CRLF bytes as CR CRLF -> CRLF.
            for value in data:
                if value == 0x0A and bmp.endswith(b"\r"):
                    bmp.pop()
                bmp.append(value)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Capture FacultyPaper screen over serial")
    parser.add_argument(
        "-p",
        "--port",
        default="",
    ...
    parser.add_argument(
        ...
        "--out",
        default="",
        help="output .bmp path (default: artifacts/screens/facultypaper-<utc>.bmp)",
    )
    ...
    if args.out.strip():
        ...
        out_path = DEFAULT_OUT / f"facultypaper-{stamp}.bmp"
