#!/usr/bin/env bash
# Create or fix astrolabe.castalia.institute -> GitHub Pages (DNS-only CNAME).
# Requires: curl, jq
#
# Auth (either):
...
#
# Usage:
#   ./scripts/cf-dns-astrolabe-github-pages.sh
#   DRY_RUN=1 ./scripts/cf-dns-astrolabe-github-pages.sh

set -euo pipefail

DRY_RUN="${DRY_RUN:-0}"
ZONE_NAME="${ZONE_NAME:-castalia.institute}"
RECORD_NAME="${RECORD_NAME:-astrolabe}"
