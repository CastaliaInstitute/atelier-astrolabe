#!/usr/bin/env bash
# Build/flash the M5 AtomS3R FacultyAtom ESP-IDF project under facultyatom/.
set -euo pipefail
...
facultyatom_idf_python() {
  if [[ -n "${FACULTYATOM_IDF_PYTHON:-}" && -x "${FACULTYATOM_IDF_PYTHON}" ]]; then
    echo "${FACULTYATOM_IDF_PYTHON}"
    return 0
  fi
  local candidate
  for candidate in \
...
cd "${ROOT}/facultyatom"
IDF_PY=(idf.py)
if resolved_py="$(facultyatom_idf_python)"; then
  IDF_PY=("${resolved_py}" "${IDF_PATH}/tools/idf.py")
  echo "facultyatom_build: using ${resolved_py}" >&2
fi
